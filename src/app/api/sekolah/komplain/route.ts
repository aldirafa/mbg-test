...
// Komplain ringkas terikat ke satu distribusi (mis. porsi kurang / kualitas turun).
// Ini BUKAN sistem pengaduan trackable penuh (entitas pengaduan menyusul di Fase 2).
export async function POST(req: Request) {
  ...
  const { distribusiId, catatan } = parsed.data;
  ...
  const distribusi = await prisma.distribusi.update({
    ...
    data: {
      status: "bermasalah",
      catatan,
      waktuTerima: existing.waktuTerima ?? new Date(),
    },
